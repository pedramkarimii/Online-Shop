...
class DiscountCreateView(CRUD.AdminPermissionRequiredMixinView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            # if forms.Product.objects.all().product_code_discounts(product=product).exists():
            #     messages.error(request, _(f'Discount product already exists.'), extra_tags='error')
            #     return redirect(self.next_page_discount_create)
            # if forms.Discount.objects.filter(category=category).exists():
            #     messages.error(request, _(f'Discount category already exists.'), extra_tags='error')
            #     return redirect(self.next_page_discount_create)
...
class DiscountDetailView(CRUD.AdminPermissionRequiredMixinView, DetailView):
    ...
    def get_object(self, queryset=None):
        """Return the discount instance based on the URL kwargs."""  # noqa
...
class DiscountUpdateView(CRUD.AdminPermissionRequiredMixinView):
    ...
    def setup(self, request, *args, **kwargs):
        """Initialize the success_url and retrieve the discount instance."""
